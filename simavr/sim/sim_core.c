...
static void
_avr_inst_opcode_xlat_d5rYZ_q6(
	avr_t * avr, 
	uint32_t opcode, 
	uint32_t * extend_opcode;
	uint8_t handler)
{
	...
	*extend_opcode = _make_opcode_h8_0r8_1r8_2r8(handler, d, rYZ, q);
}

#define AVR_INST_OPCODE_XLAT_D5rYZ_Q6(_handler) \
		_avr_inst_opcode_xlat_d5rYZ_q6(avr, opcode, &extend_opcode, _handler)
	...
	uint8_t handler)
{
	...
	*extend_opcode = _make_opcode_h8_0r8_1r8_2r8(handler, h, k, 0);
}

#define AVR_INST_OPCODE_XLAT_H4K8(_handler) \
		_avr_inst_opcode_xlat_h4k8(avr, opcode, &extend_opcode, _handler)
...
//	const int16_t o = ((int16_t)(opcode << 4)) >> 3; // CLANG BUG!
	...
	*extend_opcode = (handler << 24) | (ea & ((1 << 24) - 1));
}

#define AVR_INST_OPCODE_XLAT_O12(_handler) \
		_avr_inst_opcode_xlat_o12(avr, opcode, &extend_opcode, _handler, new_pc);
	...
	uint8_t handler)
{
	...
	*extend_opcode = _make_opcode_h8_0r8_1r8_2r8(handler, p, k, 0);
}

#define AVR_INST_OPCODE_XLAT_P2K6(_handler) \
		_avr_inst_opcode_xlat_p2k6(avr, opcode, &extend_opcode, _handler)
...
static avr_inst_decode_elem_t _avr_inst_opcode_table[] =  {
	{ -1, -1, _avr_inst_decode_one , "" }, // yes virginia, sometimes you can have a free lunch.
	INST_ESAC_TABLE
};
